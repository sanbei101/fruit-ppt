...
| 层类型 | 配置 | 输出尺寸 | 说明 |
|--------|------|----------|------|
| Conv2d | 3→16, k=3 | 100×100×16 | 提取低级特征 |
| MaxPool | 2×2 | 50×50×16 | 降维，保留主要特征 |
| Conv2d | 16→32, k=3 | 50×50×32 | 提取高级特征 |
| MaxPool | 2×2 | 25×25×32 | 再次降维 |
| Linear | 256→10 | 10 | 分类输出 |

**训练配置**:

- `CrossEntropyLoss` - 多分类交叉熵损失
- `Adam(lr=0.001)` - 自适应学习率优化器
- `epochs=5` - 快速收敛只需 5 轮
